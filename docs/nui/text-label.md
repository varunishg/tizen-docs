...
- Font Style is a JSON formatted string with the font's style. Possible *key, value* pairs are:
...
- Point Size is a float with the font's size in points. To get the point size from pixels, could use the formula: <em>point_size = 72 * pixels / vertical_dpi</em>
